...
import { insertRegistrationSchema, insertUserSchema, insertWithdrawalSchema } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/users/:id", async (req, res) => {
    try {
      ...
      console.log(`[PATCH] Updating user ${req.params.id}. Payload:`, { name, avatarLength: avatar?.length, avatarValue: avatar });
  ...
  app.post("/api/withdrawals", async (req, res) => {
    try {
      ...
      if (parseFloat(user.availableBalance || "0") < parseFloat(withdrawalData.amount)) {
        return res.status(400).json({ message: "Insufficient balance" });
      }

      // Check for pending withdrawals
      const pending = await storage.getCheckPendingWithdrawal(req.session.userId);
      if (pending) {
      ...
      // Update user balance (deduct pending amount)
      // Note: Typically you deduct from available and move to pending, or just lock it. 
      // For now, we will deduct from available balance to prevent double spending.
      await storage.updateUserEarnings(req.session.userId, `-${withdrawalData.amount}`);
