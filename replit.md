...
- 2026-07-20 (re-import, fresh empty DB): `npm install` + `npx drizzle-kit push --force` (no conflicts, clean apply). Workflow restarted on port 5000, landing page renders correctly (only expected 401 from unauthenticated session check on load). `postgresql-16` module retained in `.replit`. Founder account and system_config seed still pending.
