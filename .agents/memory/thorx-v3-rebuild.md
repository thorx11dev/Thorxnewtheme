...
## Confirmed real gaps
None open as of fifth-pass verification (2026-07-15). Prior "spec-complete" claims should not be trusted at face value; always re-trace call sites, don't just grep for keywords.

## Fifth-pass findings (2026-07-15) — no defects, one false-positive resolved
Two parallel explorer subagents independently re-derived all 10 Appendix A invariants and all core money-math formulas straight from spec text + live code (not from this memory file). Result: all 10 invariants PASS, all money math MATCHES spec. `tsc --noEmit` clean, workflow restarts cleanly, live curl regression against the dev domain confirmed invariant #10 (PATCH `/api/admin/users/:id/ps` rejects missing/short `reason` with 400, accepts ≥5 chars, writes `ADMIN_PS_ADJUSTMENT` to `audit_logs`) and invariant #7 (E-Rank floor holds at `performanceScore=0`).
- **False positive to watch for**: one subagent flagged invariant #3 ("Vault"/"Locked Points" never in user-facing strings) as FAIL because `AdminSidebar.tsx` imports a Lucide icon literally named `Vault` and `ScratchCardModal.tsx` has an internal type field `vaultPkr`. **This is not a violation** — the spec's own invariant #3 text explicitly carves out backend/internal identifiers ("Backend column names... are internal"); icon component names and internal prop/type field names are not rendered user-facing strings, and no visible label anywhere reads "Vault" (the admin nav item's `name` is "GUILDS"). Don't re-flag this without checking whether the matched string is actually rendered as user-visible text.
- Guild weekly bonus pool distribution (`guild-reset.ts`) uses plain JS float + `Math.round`/`toFixed(2)`, not `Decimal` — confirmed spec-sanctioned, not a gap: spec's QA checklist explicitly says "Proportional distribution is mathematically correct (sum ≈ pool, rounding accepted)". The strict-Decimal requirement (Critical #3 from the earlier audit) applies to `recordEarnEvent`/withdrawal math/card draws, not this distribution step.
